...
object MultiplePublish {
  ...
  def multiplePublish(queueName: String, channel: Channel): Unit = {
    (0 until 5).foreach { p =>
      val props = new BasicProperties().builder().correlationId(UUID.randomUUID().toString).replyTo(queueName).build()
